...
- 说明：
  - 用于注册和更新邮箱场景
  - 如果邮箱已被使用，会返回错误（注册和更新邮箱都不允许使用已被使用的邮箱）
...
```json
{
  ...
  "email": "string",        // 可选，新邮箱（更新邮箱时必填）
  "email_code": "string"   // 可选，邮箱验证码（更新邮箱时必填，6位数字）
}
```
- 说明：
  - 所有字段均为可选，只更新提供的字段
  - 更新邮箱需要验证码：先调用 `/api/v1/users/send-verification-code` 发送验证码到新邮箱，然后在更新请求中同时提供 `email` 和 `email_code`
  - 不允许修改积分（`points` 字段会被拒绝）
...
- 查询参数：
  ...
  - **精确查询：**
    - `id` (可选)：用户ID
    - `username` (可选)：用户名（精确匹配）
    - `email` (可选)：邮箱（精确匹配）
    - `email_verified` (可选，true/false)：邮箱是否已验证
  - **模糊查询：**
  ...
  - **排序：**
    - `order_by` (可选)：排序字段（id, username, email, points, created_at, updated_at）
    - `order` (可选，默认desc)：排序方向（asc, desc）
- 说明：
  - 所有查询参数均为可选，可组合使用
  - 如果不指定 `order_by`，按数据库默认顺序返回
  - 查询条件之间为 AND 关系
...
- 查询参数：
  ...
  - **范围查询：**
    - `created_at_min` (可选)：创建时间最小值（>=，Unix时间戳）
    - `created_at_max` (可选)：创建时间最大值（<=，Unix时间戳）
    - `updated_at_min` (可选)：更新时间最小值（>=，Unix时间戳）
    - `updated_at_max` (可选)：更新时间最大值（<=，Unix时间戳）
  - **排序：**
    - `order_by` (可选)：排序字段（id, username, created_at, updated_at）
    - `order` (可选，默认desc)：排序方向（asc, desc）
- 说明：
  - 所有查询参数均为可选，可组合使用
  - 如果不指定 `order_by`，按数据库默认顺序返回
